...
export default function ImageDownload() {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const fileContent = selectedImage.file_content || selectedImage.fileContent || selectedImage.filePath || '';
      const fileName = selectedImage.file_name || selectedImage.fileName || 'image.jpg';

      if (serviceId && templateId && publicKey) {
        try {
          // Generate a clean Web Link to download via query parameter rather than sending huge base64 in email
    ...
    <div style={styles.card} className="glass-panel">
      <div style={styles.cardPreviewContainer}>
        {loadingImg ? (
          ...
          <img src={imageSrc || 'https://images.unsplash.com/photo-1618005182384-a83a8bd57fbe?q=80&w=600&auto=format&fit=crop'} alt={image.title} style={styles.cardImage} />
        )}
...
const styles = {
  ...
  modalOverlay: {
    ...
    right: 0,
    bottom: 0,
    backgroundColor: 'rgba(3, 2, 7, 0.75)',
...
// Add animations directly to head
